...
class TestImports:
    ...
    def test_version_available(self):
        ...
        assert minio_file.__version__ == "2025.1.3"
    ...
    def test_module_functions_exist(self):
        ...
        from minio_file import transport, minio_file as mf
...
class TestCLI:
    ...
    def test_cli_command_exists(self):
        ...
        result = subprocess.run(['which', 'minio-file'], 
                              capture_output=True, text=True)
        if result.returncode != 0:
            # Try Windows where command
            result = subprocess.run(['where', 'minio-file'], 
                                  capture_output=True, text=True)
    ...
    def test_cli_help(self):
        ...
        try:
            ...
            for cmd in help_commands:
                ...
                if len(output) > 0:
                    if any(word in output.lower() for word in ['help', 'usage', 'command', 'config', 'credential', 'endpoint']):
                        success = True
                        break
